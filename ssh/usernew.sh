...
echo "Checking VPS"
clear
#slowdns data (guarded: only if installed)
if [ -f "/root/nsdomain" ] && [ -f "/etc/slowdns/server.pub" ]; then
sldomain=$(cat /root/nsdomain)
slkey=$(cat /etc/slowdns/server.pub)
slnsport="ALL Port (22, 443, 143)"
else
sldomain="-"
slkey="-"
slnsport="-"
fi
#http custom data (guarded: only if installed)
if [ -f "/root/udp/config.json" ] && [ -f "/root/udp/udp-custom" ]; then
hcport=$(grep -o '"listen": ":[0-9]*"' /root/udp/config.json | grep -o '[0-9]*$')
[ -z "$hcport" ] && hcport="36712"
hchost=$(cat /etc/xray/domain 2>/dev/null || cat /etc/v2ray/domain 2>/dev/null)
else
hcport="-"
hchost="-"
fi
...
echo -e "Port NS     : $slnsport" | tee -a /etc/log-create-ssh.log
echo -e "Nameserver  : $sldomain" | tee -a /etc/log-create-ssh.log
echo -e "Pubkey      : $slkey" | tee -a /etc/log-create-ssh.log
echo -e "HTTP Custom : $hcport" | tee -a /etc/log-create-ssh.log
echo -e "HTTP Custom : $hchost:$hcport@$Login:$Pass" | tee -a /etc/log-create-ssh.log
echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m" | tee -a /etc/log-create-ssh.log
